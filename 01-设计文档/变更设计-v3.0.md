...
3. **新模块：分析单（M04）**：承接从退货单移出的抽样与报废功能，按「退货单 × 分析师」自动分组，分析员仅可见自己的分析单
...
| 抽样功能 | 在退货单中提供 | **移除**，迁移至分析单（M04） |
| 报废功能 | 在退货单中提供 | **移除**，迁移至分析单（M04） |
...
| M03 售后件管理 | 字段调整 | 移除失效类别/投诉类型，新增客户失效类型/博世失效类型，分析师必填，编号格式加 `-` |
| M06 审批管理 | 联动调整 | 报废审批确认 Tab 改为关联分析单（`ANALYSIS_ORDER_ID`） |
| M04 分析单管理 | **新建** | 全新模块，详见第三节 |
| 数据库 | 结构变更 | 见第四节 Flyway 清单 |
| 权限矩阵 | 更新 | M04 分析单按角色隔离可见范围 |
