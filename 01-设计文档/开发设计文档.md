...
> v5.2 更新内容（2026-08-07）：
> - **导入创建/更新时间规则调整**：导入生成的退货单/售后件 `createdAt`（创建时间）与 `updatedAt`（更新时间）统一取**导入执行当日**（由 JPA 审计 `@CreatedDate`/`@LastModifiedDate` 自动填充），不再按「文件名年份 + 行内月份 + 1 日」合成。作废 v4.6 的旧合成规则及跨年结转逻辑（`ImportService.resolveImportCreatedAt` 等已删除）。
> - **售后件导入收货/投诉日期改为合成**：售后件报表 Excel 无「收货日期」列，自动创建退货单的 `receiveDate`（收货时间）/`complaintDate`（投诉日期）改为按「**文件名年份 + 「月份」列值 + 1 号**」合成（如文件名 `..._2025` + 月份列 `12` → `2025-12-01`）。`PartImportParser` 新增「月份」列映射（优先中文表头「月份」/英文「Month」子串匹配；回退英文表头精确等于「NO」的列，避免与 QC No / MRB No / Return No 混淆），单元格取纯数字 1~12。因收货/投诉日期按退货单共享，**只要该退货单下任一退货件的月份列有有效值即可**。文件名无年份、或全单月份列均无有效值时，该退货单创建失败（连带其下售后件记为失败），不再回退到当天。
> - 退货单导入流程（Excel 本身含「收货时间」列）不受影响，`receiveDate`/`complaintDate` 仍取该列真实日期。
>
> v5.1 更新内容（2026-06-22）：
> - **修复"新建退件残留上次表单"问题**：根因是 Vue Router 在 `return-orders/new` 与 `return-orders/:id/edit`（同一 `OrderForm.vue` 组件）之间复用组件实例、`onMounted` 不再触发；叠加 wujie 子应用 keep-alive，导致表单状态残留。修复：`MainLayout.vue` 的 `<router-view>` 增加 `:key="$route.fullPath"` 强制按路由重新挂载；`OrderForm.vue`/`PartForm.vue` 抽出 `getInitialForm()`/`resetForm()`，`onMounted` 新建分支显式重置表单（防御性补强）
> - **返回箭头改为语义化导航**：所有详情/表单页左上角返回箭头（`OrderDetail`/`OrderForm`/`PartDetail`/`PartForm`/`AnalysisOrderDetail`/`AnalysisForm`）由 `router.back()` 改为 `router.push(逻辑父级)`，不再依赖浏览器历史栈；逻辑父级映射：退货单类→`/return-orders`、售后件类→`/return-parts`、分析单详情→`/analysis-orders`、精分析表单→`/return-parts/:id`（回零件详情）
> - **修复"从退货单详情创建退件后，再次创建残留旧表单"问题**：该流程是跨 tab 的 `jump`（退货单→售后件 tab）。AEP 复用同一售后件 tab，若 jump 的 fullPath 与上次完全相同，wujie/vue-router 去重导致不重新挂载、keep-alive 冻结的旧表单被原样恢复。AEP 宿主经 `window.$wujie.props` 只暴露 `{jump, locale, microData, path, state, token, userProfile}`，**无关闭 tab 接口**，子应用无法自行关闭 tab。修复：①`OrderDetail.vue` 创建退件的 jump 带唯一时间戳 query（`_t=Date.now()`），强制每次 fullPath 不同 → 必然重新挂载；②`PartForm.vue` 提交/暂存成功且来自退货单详情时，先将本 tab `router.push('/return-parts')` 离开新建页，再 `jump` 回退货单详情（合理的"创建完成"状态，并在任何 wujie 同步模式下都安全）
>
> v5.0 更新内容（2026-05-29）：
> - **全面对齐代码**：状态枚举从中文格式改为英文下划线格式（如 `初分析/进行中` → `in_initial_analysis`）；退货单状态补充 `registered`；售后件状态扩展为 9 个；分析单状态扩展为 6 个
> - 数据库表结构全面更新：`FAILURE_TYPE` → `COMPLAINT_TYPE`；`REPAIR_STATION_LOCATION` 拆分为 `REPAIR_STATION` + `COMPLAINT_LOCATION`；移除不存在的独立表（APMS_PART_IMAGE、APMS_REPORT_ATTACHMENT、APMS_BUSINESS_UNIT、APMS_FAILURE_CATEGORY、APMS_SAMPLING、APMS_SAMPLING_DETAIL、APMS_REPORT_SNAPSHOT）；新增 APMS_SMB_CONFIGURATION、APMS_IMPORT_LOG_DETAIL
> - API 路径更新：`/api/v1/templates` → `/api/v1/report-templates`；`/api/v1/parts/{id}/analysis` → `/api/v1/analysis-reports`；新增 SMB 配置、导入明细等接口
> - ER 图完全重写
> - 步骤条映射更新为新状态格式
> - 统计报表改为直接聚合查询（不使用快照表）
>
...
> - 退货单客户标识从文件名 `DATA_..._Amount` 片段提取并写入退货单 `CUSTOMER`；文件名不符合格式时通过导入日志提示用户；快递单号为空时自动生成占位符 `IMPORT-{退货单号}`
...
**新建/暂存售后件：**
- 输入：orderId、partCode（零件号，必须匹配 APMS_PART_CODE.PART_CODE）、businessUnit、productPlatform（由后端根据 partCode 自动查 APMS_PART_CODE 填充）、customerFailureType（客户失效类型）、complaintType（退货类型）、repairStation（修复站点）、complaintLocation（投诉地点）、responsibleEngineer、analyst（必填）、vehicleInfo（非0km时）、otherInfo（可选）
...
**照片字段上传（并发安全）：**
- `photo`（单张）/ `photolist`（多张）字段的照片上传至 SMB，相对路径写入报告 `content`
- 上传前若报告尚无 `reportId`，前端先调用保存草稿接口建报告（`ensureReportSaved`），再用该 `reportId` 关联附件路径
- **防抖与串行化**（修复连点导致重复上传、重复上传后无法删除的缺陷）：
  - 拍照组件 `CameraCapture` 的快门在异步 `canvas.toBlob` 期间通过 `capturing` 标志禁用，防止连点触发两次 `captured` 事件
  - `AnalysisReportModal` 的所有照片上传（拍照回调和文件输入两条入口）经 `serializedUpload` 串行化执行，避免并发上传对共享状态（`photoFileLists` / `form.content` / `reportId`）的读改写竞态，确保删除时 `reportId` 与照片归属一致
...
统计报表直接从业务表聚合查询（APMS_RETURN_ORDER、APMS_PART、APMS_ANALYSIS_REPORT 等），通过数据库侧聚合优化查询性能。
...
| 5 | 审批超期提醒 | `APMS_ANALYSIS_ORDER.STATUS = 'pending_approval'` 且 `SYSDATE - STATUS_CHANGED_AT ≥ approval-overdue-days` | 3天/次 | QMC_Leader | — | 批次级：当批次内所有抽样件报告提交后批次置 `pending_approval`；按批次（`ANALYSIS_ORDER_ID`）去重，邮件含退货单号+分析师；v5.3 修复（原误查 Part 表） |

**通知频率控制：**

定时任务通过查询 `APMS_NOTIFICATION_LOG` 表判断是否需要发送：当某售后件某类通知的最后一次**成功**发送（`STATUS='SENT'`）时间距今超过配置的频率天数时，才触发发送。

```
发送条件 = 满足扫描条件
    AND (该 PART_ID + NOTIFICATION_TYPE 无 STATUS='SENT' 的历史记录
...
erDiagram
    ...
    APMS_PART {
        ...
        string BUSINESS_UNIT
        string PRODUCT_PLATFORM
        string CUSTOMER_FAILURE_TYPE
        string BOSCH_FAILURE_TYPE
        string COMPLAINT_TYPE
    ...
    APMS_ANALYSIS_ORDER {
        ...
        string ANALYST
        string STATUS
        string WORKON_SCRAP_NO
        timestamp SCRAP_STARTED_AT
        timestamp STATUS_CHANGED_AT
    }
    ...
    APMS_NOTIFICATION_LOG {
        ...
        string ANALYSIS_ORDER_ID
        string STATUS
        timestamp SENT_AT
        string ERROR_MESSAGE
    }
    APMS_EMAIL_CONFIGURATION {
        string ID PK
        string SMTP_HOST
        int SMTP_PORT
    ...
    APMS_CUSTOMER ||..o{ APMS_RETURN_ORDER : "客户引用"
    APMS_PART_CODE ||..o{ APMS_PART : "零件号/产品平台引用"
    APMS_RETURN_ORDER ||--o{ APMS_PART : "包含售后件"
    APMS_RETURN_ORDER ||--o{ APMS_ANALYSIS_ORDER : "包含分析单(UNIQUE ORDER_ID+ANALYST)"
    APMS_PART ||--o| APMS_ANALYSIS_REPORT : "精分析报告"
    APMS_PART ||--o{ APMS_OCR_TASK : "OCR任务"
    APMS_PART ||--o{ APMS_NOTIFICATION_LOG : "通知记录"
    APMS_REPORT_TEMPLATE ||--o{ APMS_ANALYSIS_REPORT : "被引用"
    APMS_IMPORT_RECORD ||--o{ APMS_IMPORT_LOG_DETAIL : "导入明细"
```
...
| CUSTOMER_ID | VARCHAR2(36) | — | 客户ID |
| CUSTOMER | VARCHAR2(100) | NOT NULL | 客户名称（冗余用于显示） |
...
| COMPLAINT_TYPE | VARCHAR2(20) | NOT NULL | 投诉类型（BA10-BA79标准代码）。BA20（0-km, uninstalled）代表0km退货单，不能抽样，只能走报废流程。仅 BA40 可触发抽样。 |
| OTHER_INFO | VARCHAR2(500) | — | 其他信息 |
| STATUS | VARCHAR2(50) | NOT NULL | 状态枚举（见 3.4），如 `draft`、`submitted`、`registered`、`scrapped` |
...
| ORDER_ID | VARCHAR2(36) | NOT NULL | 关联退货单 |
| PART_CODE | VARCHAR2(50) | NOT NULL | 零件号 |
| BUSINESS_UNIT | VARCHAR2(50) | — | 业务单元名称 |
| PRODUCT_PLATFORM | VARCHAR2(50) | — | 产品平台名称（通过 APMS_PART_CODE 查询匹配） |
| PART_PRODUCTION_DATE | DATE | — | 零件生产日期 |
| PRODUCTION_SHIFT | VARCHAR2(20) | — | 生产班次 |
| CUSTOMER_FAILURE_TYPE | VARCHAR2(50) | — | 客户失效类型（NVH / 功能 / 外观） |
| BOSCH_FAILURE_TYPE | VARCHAR2(20) | — | 博世失效类型（BA 代码） |
| COMPLAINT_TYPE | VARCHAR2(20) | — | 投诉类型（同退货单 COMPLAINT_TYPE，冗余存储） |
| VEHICLE_PRODUCTION_DATE | DATE | — | 汽车生产日期 |
| VEHICLE_PURCHASE_DATE | DATE | — | 汽车购买日期 |
| VEHICLE_FAILURE_DATE | DATE | — | 汽车失效日期 |
| VEHICLE_VIN | VARCHAR2(255) | — | VIN 码 |
| VEHICLE_MILEAGE | NUMBER(10) | — | 行驶里程（km） |
| CUSTOMER_DESCRIPTION | VARCHAR2(500) | — | 客户失效描述 |
| OTHER_DESCRIPTION | VARCHAR2(500) | — | 其他描述 |
| OTHER_INFO | VARCHAR2(500) | — | 其他信息 |
| REPAIR_STATION | VARCHAR2(100) | — | 修复站点 |
| COMPLAINT_LOCATION | VARCHAR2(100) | — | 投诉地点 |
| RESPONSIBLE_ENGINEER | VARCHAR2(100) | — | 责任工程师（存 loginName） |
| ANALYST | VARCHAR2(100) | NOT NULL | 分析师（存 loginName，必填） |
| QC_NO | VARCHAR2(50) | — | QC 单号（精分析审批通过后录入） |
| IS_SAMPLE | NUMBER(1) | DEFAULT 0 | 是否被选为抽样样本 |
| IMAGES | VARCHAR2(2000) | — | 图片路径列表（JSON 数组字符串） |
| STATUS_CHANGED_AT | TIMESTAMP | — | 当前状态进入时间（用于预警计算） |
| STATUS | VARCHAR2(50) | NOT NULL | 状态枚举（见 3.4），如 `in_initial_analysis` |
| CREATED_BY | VARCHAR2(100) | NOT NULL | 创建人 |
| CREATED_AT | TIMESTAMP | NOT NULL | 创建时间 |
| UPDATED_BY | VARCHAR2(100) | — | 更新人 |
| UPDATED_AT | TIMESTAMP | — | 更新时间 |
...
**字段说明：**
- `PART_CODE` 必须存在于 `APMS_PART_CODE` 且 `ENABLED=1`，选择后自动带出 `PRODUCT_PLATFORM`
- `CUSTOMER_FAILURE_TYPE` 由用户从下拉选择（NVH / 功能 / 外观），`BOSCH_FAILURE_TYPE` 在精分析报告中选择 BA 代码
- `REPAIR_STATION` 和 `COMPLAINT_LOCATION` 为两个独立字段
- `IMAGES` 存储图片 NAS 路径的 JSON 数组字符串，不再使用独立的 APMS_PART_IMAGE 表
...
| ID | VARCHAR2(36) | PK | UUID |
| ORDER_ID | VARCHAR2(36) | NOT NULL | 关联退货单 |
| ANALYST | VARCHAR2(100) | NOT NULL | 分析师（loginName） |
| STATUS | VARCHAR2(50) | NOT NULL | 见分析单状态枚举（3.4） |
| WORKON_SCRAP_NO | VARCHAR2(100) | — | WorkON 报废单号，确认报废时必填 |
| SCRAP_STARTED_AT | TIMESTAMP | — | 报废开始时间，首次标记报废时记录，之后不再修改 |
...
|------|------|------|------|
| ID | VARCHAR2(36) | PK | UUID |
| PART_ID | VARCHAR2(36) | NOT NULL | 关联售后件 |
| TEMPLATE_ID | VARCHAR2(36) | NOT NULL | 使用的模板 |
| CONTENT | VARCHAR2(4000) | — | 用户填写内容 JSON |
| SUMMARY | VARCHAR2(500) | — | 报告摘要 |
| STATUS | VARCHAR2(20) | NOT NULL | draft/submitted/approved/rejected |
| ATTACHMENTS | VARCHAR2(1000) | — | 附件路径列表（JSON 数组字符串） |
...
| FILE_PATH | VARCHAR2(500) | NOT NULL | 原始 Excel 存储路径 |
| FILE_NAME | VARCHAR2(200) | NOT NULL | 原始文件名 |
| FIELD_DEFINITIONS | CLOB | — | 占位符解析结果 JSON 数组（@Lob 注解） |
...
|------|------|------|------|
| ID | VARCHAR2(36) | PK | UUID |
| NAME | VARCHAR2(100) | UNIQUE, NOT NULL | 客户名称（唯一） |
| CODE | VARCHAR2(50) | UNIQUE | 客户代码（唯一） |
| CREATED_BY | VARCHAR2(100) | NOT NULL | 创建人 |
| CREATED_AT | TIMESTAMP | NOT NULL | 创建时间 |
| UPDATED_BY | VARCHAR2(100) | — | 更新人 |
| UPDATED_AT | TIMESTAMP | — | 更新时间 |
...
| BUSINESS_UNIT | VARCHAR2(100) | — | 业务单元 |
| PRODUCT_PLATFORM | VARCHAR2(100) | — | 产品平台名称 |
| CREATED_BY | VARCHAR2(100) | NOT NULL | 创建人 |
| CREATED_AT | TIMESTAMP | NOT NULL | 创建时间 |
| UPDATED_BY | VARCHAR2(100) | — | 更新人 |
| UPDATED_AT | TIMESTAMP | — | 更新时间 |
...
|------|------|------|------|
| ID | VARCHAR2(36) | PK | UUID |
| PART_ID | VARCHAR2(36) | — | 关联的 Part ID（可空，新建模式后绑定） |
| FILE_PATH | VARCHAR2(500) | — | 临时文件路径 |
| STATUS | VARCHAR2(20) | NOT NULL | CREATED / PROCESSING / SUCCESS / FAILED |
| RESULT_JSON | CLOB | — | OCR 识别结果 JSON |
| ERROR_MESSAGE | VARCHAR2(500) | — | 失败原因 |
| CREATED_BY | VARCHAR2(100) | NOT NULL | 创建人 |
| CREATED_AT | TIMESTAMP | NOT NULL | 创建时间 |
| UPDATED_BY | VARCHAR2(100) | — | 更新人 |
| UPDATED_AT | TIMESTAMP | — | 更新时间 |
...
|------|------|------|------|
| ID | VARCHAR2(36) | PK | UUID |
| IMPORT_TYPE | VARCHAR2(50) | NOT NULL | 导入类型（return_order / part） |
| FILE_NAME | VARCHAR2(255) | NOT NULL | 文件名 |
| STATUS | VARCHAR2(20) | NOT NULL | processing / completed / deleting / timeout / rolled_back |
| TOTAL_COUNT | NUMBER | NOT NULL | 总行数 |
| SUCCESS_COUNT | NUMBER | NOT NULL | 成功行数 |
| FAIL_COUNT | NUMBER | NOT NULL | 失败行数 |
| FAIL_LOGS | CLOB | — | 失败日志 JSON 数组 |
| IMPORT_LOGS | CLOB | — | 完整导入日志 |
| CREATED_BY | VARCHAR2(100) | NOT NULL | 操作人 |
...
|------|------|------|------|
| ID | VARCHAR2(36) | PK | UUID |
| IMPORT_ID | VARCHAR2(36) | NOT NULL | 关联导入记录 |
| FILE_NAME | VARCHAR2(255) | — | 文件名 |
| ROW_NUMBER | NUMBER | — | 行号 |
| STATUS | VARCHAR2(20) | NOT NULL | success / error |
| ERROR_CODE | VARCHAR2(50) | — | 错误代码 |
| ERROR_MESSAGE | VARCHAR2(500) | — | 错误信息 |
| RAW_DATA | CLOB | — | 原始数据 |
| ORDER_ID | VARCHAR2(36) | — | 创建的退货单 ID |
| ORDER_NUMBER | VARCHAR2(50) | — | 退货单号 |
| ORDER_CREATED | VARCHAR2(1) | — | 是否新建退货单 |
| PART_ID | VARCHAR2(36) | — | 创建的售后件 ID |
| PART_CODE | VARCHAR2(50) | — | 零件号 |
| PART_NUMBER | VARCHAR2(50) | — | 售后件编号 |
| QC_NO | VARCHAR2(50) | — | QC 号 |
| CREATED_AT | TIMESTAMP | NOT NULL | 创建时间 |
...
|------|------|------|------|
| ID | VARCHAR2(36) | PK | UUID |
| SMTP_HOST | VARCHAR2(255) | NOT NULL | SMTP 服务器地址 |
| SMTP_PORT | NUMBER | DEFAULT 25 | SMTP 端口（25/587/465） |
| SMTP_USERNAME | VARCHAR2(255) | — | SMTP 认证用户名 |
| SMTP_DOMAIN | VARCHAR2(255) | — | SMTP 认证域 |
| EMAIL_FROM | VARCHAR2(255) | NOT NULL | 发件人邮箱地址 |
| EMAIL_FROM_DISPLAY_NAME | VARCHAR2(255) | — | 发件人显示名称 |
| EMAIL_PASSWORD | VARCHAR2(255) | NOT NULL | 邮箱密码 |
| ENABLE_SSL | NUMBER(1) | DEFAULT 0 | 是否启用 SSL 加密 |
...
|------|------|------|------|
| ID | VARCHAR2(36) | PK | UUID |
| HOST | VARCHAR2(255) | NOT NULL | SMB 服务器地址 |
| SHARE_NAME | VARCHAR2(255) | NOT NULL | 共享名称 |
| DOMAIN | VARCHAR2(100) | — | 域 |
| SMB_USER | VARCHAR2(100) | NOT NULL | SMB 用户名 |
| SMB_PASSWORD | VARCHAR2(255) | NOT NULL | SMB 密码 |
| PREFIX | VARCHAR2(100) | NOT NULL | NAS 根目录前缀 |
| ENV | VARCHAR2(50) | NOT NULL | 环境（prod/test） |
| ENABLED | NUMBER(1) | DEFAULT 1 | 是否启用 |
| CREATED_BY | VARCHAR2(100) | NOT NULL | 创建人 |
| CREATED_AT | TIMESTAMP | NOT NULL | 创建时间 |
| UPDATED_BY | VARCHAR2(100) | — | 更新人 |
...
|------|------|------|------|
| ID | VARCHAR2(36) | PK | UUID |
| PART_ID | VARCHAR2(36) | FK→APMS_PART | 关联售后件（APPROVAL_REMINDER 时为 NULL） |
...
| ANALYSIS_ORDER_ID | VARCHAR2(36) | FK→APMS_ANALYSIS_ORDER | 关联分析批次，仅 APPROVAL_REMINDER 使用；其他类型为 NULL |
| STATUS | VARCHAR2(10) | NOT NULL | 发送状态：`SENT`（全部发送成功）/ `FAILED`（任一收件人或 CC 发送失败）。频率去重仅认 `SENT`，`FAILED` 不计，下轮 cron 自动重试 |
| SENT_AT | TIMESTAMP | NOT NULL | 发送/尝试时间（FAILED 记录为尝试时刻） |
| ERROR_MESSAGE | VARCHAR2(500) | — | 失败明细（地址 + 错误信息，分号分隔；STATUS=FAILED 时记录） |
...
APMS_PART_CODE          (ID VARCHAR2(36) PK, PART_CODE VARCHAR2(100) UNIQUE NOT NULL,
...
| APMS_ANALYSIS_ORDER | ORDER_ID, ANALYST | UNIQUE | 每退货单每分析师唯一 |
| APMS_ANALYSIS_REPORT | PART_ID | 普通 | 售后件关联查询 |
| APMS_NOTIFICATION_LOG | PART_ID, NOTIFICATION_TYPE | 复合 | 定时任务查询发送频率 |
...
spring:
  jackson:
    time-zone: Asia/Shanghai     # 全局 Jackson 时区（与 JVM 时区保持一致）
```

#### 时区统一策略（Asia/Shanghai, UTC+8）

全系统统一使用上海时间，链路：**生成 → 存储 → API 输出 → 前端显示**。

- **JVM 时区（根治点）**：容器 `Dockerfile` 设置 `ENV TZ=Asia/Shanghai` 与启动参数 `-Duser.timezone=Asia/Shanghai`。一处生效，`LocalDateTime.now()`、Oracle JDBC 对 `TIMESTAMP` 字段的读写解释、`@Scheduled` 默认时区全部归一为上海。**业务代码中的 `LocalDateTime.now()` / `LocalDate.now()` 无需逐处修改。**
- **Jackson**：`application.yml` 配 `spring.jackson.time-zone: Asia/Shanghai`，`JacksonConfig` 同时 `mapper.setTimeZone(Asia/Shanghai)`，防御性双重保险。
- **定时任务**：`NotificationService` 的 `@Scheduled` 显式声明 `zone = "Asia/Shanghai"`，避免 JVM 时区被覆盖时任务跑偏。
- **前端**：`src/utils/dayjs.ts` 注册 utc + timezone 插件，`dayjs.tz.setDefault('Asia/Shanghai')`，由 `main.ts` 最先引入。解析后端无偏移时间字符串统一用 `formatShanghai(raw)`（内部 `dayjs.tz(raw, 'Asia/Shanghai')`）；取"当前时间/当前年"统一用 `nowShanghai()` / `dayjs.tz()`。导出文件名用 `dayjs.tz().format('YYYYMMDD')`（不再用 `new Date().toISOString()`，避免 UTC 少一天）。
- **Oracle 数据源 URL 不加 `serverTimezone`**：该参数为 MySQL 专用，Oracle Thin 驱动不识别；Oracle `TIMESTAMP` 时区由 JVM 时区决定，已由上述 JVM 参数覆盖。
- **历史数据**：不迁移。本次仅修正未来写入与显示逻辑。

> DTO 仍以 `.toString()` 输出无偏移字符串（如 `2026-01-15T10:30:00`），JVM 时区锁定后该值即上海时间，前端按上海时区解析即可正确显示。
