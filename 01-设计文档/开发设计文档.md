...
| M08 | 分析单管理 | /analysis-orders | 分析员（仅自己的）、分析主管、QMC 经理、系统管理员 |
...
- **约束条件**：BA20失效类型退货单（`FAILURE_TYPE='BA20'`）不能抽样，后端抛出异常："BA20 (0km) failure type orders cannot be sampled and must go through the scrap process directly"
...
> **说明**：退货单提交后，后续精分析、抽样、报废等流程均在分析单（M08）中进行，退货单仅保留草稿和已提交两个状态。

##### 2.2.2.4 操作流程图（必填）

```mermaid
flowchart TD
    Create[新建退货单\n填写表单（含是否为0km）] --> Choice{暂存或提交}
    Choice -->|暂存| Draft[草稿状态]
    Choice -->|提交生成编号| Initial[初分析阶段]
    Initial --> Record[录入初分析结果]
    Record --> Check0km{是否为0km退货单?}
    Check0km -->|是 0km| DirectScrap[直接进入报废流程\n跳过抽样]
    Check0km -->|否 非0km| StartSampling[发起抽样管理]
...
- **列表与查询**：支持退货单号（模糊）、零件号、业务单元、产品平台、状态、QC录入（已录/未录）等筛选
- **CRUD**：新建（暂存/提交）、编辑（受权限控制）、删除（草稿状态）
- **基础信息录入**：零件号输入后调 SAP 接口自动填充业务单元和产品平台；退货类型（BA 代码，即 COMPLAINT_TYPE）；失效类别（有失效描述时必填）；责任工程师、分析师（下拉选系统用户）；维修站号、投诉地
...
**新建/暂存售后件：**
- 输入：orderId、partCode、businessUnitId、productPlatformId、complaintType（退货类型）、failureCategoryId、customerDescription、repairStation、complaintLocation、responsibleEngineer、analyst、vehicleInfo（非0km时）
...
| 内部 | GET `/api/v1/product-platforms` | 产品平台下拉（数据字典） |
...
| 实体表 | 用途 |
|------|------|
| `APMS_RETURN_ORDER` | 退货单相关图表 |
| `APMS_PART` | 售后件、抽样件、状态分布图表 |
| `APMS_ANALYSIS_REPORT` | 分析时长、责任判定 |
| `APMS_FAILURE_CATEGORY` | 失效模式维度 |
| `APMS_BUSINESS_UNIT` / `APMS_PRODUCT_PLATFORM` | BU / 产品平台维度 |
| `APMS_CUSTOMER` | 客户维度 |

**架构注意**：`责任判定` 存于 `APMS_ANALYSIS_REPORT.CONTENT_JSON`（CLOB）。为报表查询性能，建议在 `APMS_ANALYSIS_REPORT` 新增冗余列 `RESPONSIBILITY_JUDGMENT VARCHAR2(50 CHAR)`，于报告提交时同步写入。

**Mis 计算**：派生字段，由 `FLOOR((VEHICLE_FAILURE_DATE - VEHICLE_PURCHASE_DATE) / 30)` 计算，仅适用于非 0km 投诉类型。

##### 2.2.6.6 与其他模块的交互（必填）

统计报表为只读模块，不修改其他模块数据，仅聚合查询。底部列表"查看详情"导航至 M03 售后件详情页。

##### 2.2.6.7 调用的内外部接口（必填）

见 2.2.6.2 输入/输出数据流中的 API 接口表。
...
| 操作 | 输入 | 输出 |
|------|------|------|
| 新增/编辑产品平台 | `{ name, code, businessUnitId }` | ProductPlatformDTO |
| 删除产品平台 | platformId | 无 |
| 产品平台列表 | 无 | `[{ id, name, code, businessUnitId, ... }]` |
...
| APMS_PRODUCT_PLATFORM | 产品平台数据字典（name、code、businessUnitId） |
...
| 分析管理（M03） | 业务单元、产品平台、失效类别下拉选项来源数据字典 |
...
| 内部 | GET `/api/v1/product-platforms` | 产品平台列表 |
| 内部 | POST `/api/v1/product-platforms` | 新增产品平台 |
| 内部 | PUT `/api/v1/product-platforms/{id}` | 编辑产品平台 |
| 内部 | DELETE `/api/v1/product-platforms/{id}` | 删除产品平台 |
...
erDiagram
    APMS_CUSTOMER {
        VARCHAR2 ID PK
        VARCHAR2 NAME UNIQUE
        VARCHAR2 CODE UNIQUE
        NUMBER ENABLED
        VARCHAR2 CREATED_BY
    ...
    APMS_PRODUCT_PLATFORM ..o{ APMS_PART : "产品平台引用"
...
| PRODUCT_PLATFORM | VARCHAR2(100) | NOT NULL | 产品平台名称（引用 APMS_PRODUCT_PLATFORM.NAME） |
...
| CODE | VARCHAR2(50) | UNIQUE, NOT NULL | 客户代码（唯一，用于 SAP 对接） |
| ENABLED | NUMBER(1) | DEFAULT 1 | 是否启用（软删除） |
| CREATED_BY | VARCHAR2(100) | NOT NULL | 创建人 |
| CREATED_AT | TIMESTAMP | NOT NULL | 创建时间 |
| UPDATED_BY | VARCHAR2(100) | — | 更新人 |
...
#### APMS_PRODUCT_PLATFORM（产品平台数据字典）

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| ID | VARCHAR2(36) | PK | UUID |
| NAME | VARCHAR2(100) | UNIQUE, NOT NULL | 产品平台名称 |
| CODE | VARCHAR2(20) | UNIQUE, NOT NULL | 产品平台代码 |
| BUSINESS_UNIT_ID | VARCHAR2(36) | FK→APMS_BUSINESS_UNIT | 所属业务单元 |
| ENABLED | NUMBER(1) | DEFAULT 1 | 是否启用（软删除） |
| CREATED_BY | VARCHAR2(100) | NOT NULL | 创建人 |
| CREATED_AT | TIMESTAMP | NOT NULL | 创建时间 |
| UPDATED_BY | VARCHAR2(100) | — | 更新人 |
...
APMS_BUSINESS_UNIT    (ID VARCHAR2(36) PK, NAME VARCHAR2(100) UNIQUE NOT NULL,
                        ...
                        CREATED_BY VARCHAR2(100) NOT NULL, CREATED_AT TIMESTAMP NOT NULL,
                        UPDATED_BY VARCHAR2(100), UPDATED_AT TIMESTAMP)
APMS_PRODUCT_PLATFORM (ID VARCHAR2(36) PK, NAME VARCHAR2(100) UNIQUE NOT NULL,
                        CODE VARCHAR2(20) UNIQUE NOT NULL, BUSINESS_UNIT_ID VARCHAR2(36) FK,
                        ENABLED NUMBER(1) DEFAULT 1, CREATED_BY VARCHAR2(100) NOT NULL,
                        CREATED_AT TIMESTAMP NOT NULL, UPDATED_BY VARCHAR2(100), UPDATED_AT TIMESTAMP)
...
共 **2 个**状态（精分析/抽样/报废流程已迁移至分析单 M08）：
...
| v2.0 | 2.2.6, 报表需求.md | M06 报表设计 | 完整重写 M06 节：9 张图表（3 类 KPI 卡片、玫瑰图、折线图、2 张动态堆叠柱状图、分析时长柱状图、状态柱状图）；定义动态维度参数体系、全部 API 接口及响应结构；新增 RESPONSIBILITY_JUDGMENT 冗余列建议、Mis 计算逻辑；同步新建 报表需求.md 业务需求文档 | 2026-03-04 |
...
| v2.3 | 2.2.7, 3.9 | 导入管理模块 | 新增独立导入管理模块（M07）：独立导入页面（/imports），支持历史导入记录追踪；退货单新格式导入（第1行标题/第2行表头/第3行起数据）；新建 APMS_IMPORT_RECORD 表；固定字段映射（客户=UNKNOWN，快递，数量=0，失效类型=BA40）；支持失败日志追溯 | 2026-03-13 |
| v2.4 | 2.2.5, 3.9 | 精分析模板匹配优化 | 精分析报告模板匹配改用产品类别（productCategory）替代产品平台（productPlatform）；数据库迁移：APMS_REPORT_TEMPLATE.PRODUCT_PLATFORM → PRODUCT_CATEGORY；后端实体/服务/控制器/DTO同步更新；前端组件/API/类型定义同步更新 | 2026-03-16 |
...
| v2.6 | 2.2.7, 3.3, 数据库 | 邮件服务器配置功能 | 新增邮件服务器配置功能：数据库表 APMS_EMAIL_CONFIGURATION；后端实体 EmailConfiguration、EmailConfigurationService、EmailController、EmailService；前端邮件配置组件；支持 SMTP 认证（端口25/587/465）、STARTTLS/SSL 加密、域认证；测试连接功能 | 2026-03-17 |
| v3.0 | 变更设计-v3.0.md | 分析单/退货单简化/售后件字段 | 退货单 FAILURE_TYPE 重命名为 COMPLAINT_TYPE，仅 BA40 可抽样，移除抽样/报废功能，状态简化为 2 个；售后件基础信息移除失效类别/投诉类型，信息卡新增 LLM 推导的客户失效类型（NVH/功能/外观），精分析新增博世失效类型（BA codes），编号段间加 `-`，分析师改为必填；新增分析单模块（M08），承接抽样/报废，按退货单×分析师自动分组，分析员仅见自己的分析单；详细设计见 `变更设计-v3.0.md` | 2026-03-18 |
