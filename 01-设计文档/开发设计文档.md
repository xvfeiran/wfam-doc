...
> v4.14 更新内容：
> - 前端 wujie 嵌入模式下读取当前用户信息：解析 `accessToken`（JWT）payload 提取 `roleNames` 和 `username`，用于按钮可见性控制和分析师默认筛选
> - 新增 `src/utils/jwt.ts`（JWT 解析工具）、扩展 `userInfo` store、消除 `usePermissions.ts` 中所有 TODO
>
> v4.13 更新内容：
...
| 2 | 0公里退货通知 | 售后件创建/提交时 `FAILURE_TYPE` 为 0km 类型（BA10/BA20/BA21/BA30/BA31） | `RESPONSIBLE_ENGINEER` | — | 0公里退货提醒 |
