...
| IDM 角色名称 | 描述 | 菜单权限 |
|------|------|------|
| W_RBCC_AEP_WFAM_Customer_Quality_ENG | 客户质量工程师 | M01 M02 M03 M04 M05 M07 |
...
- **基础信息录入**：零件号为文本输入，输入后自动匹配数据字典（APMS_PART_CODE）；匹配成功时产品平台自动填充且不可编辑，匹配失败时产品平台为空（两字段均为必填，未匹配则无法保存）；退货类型（BA 代码，即 COMPLAINT_TYPE）；失效类别（有失效描述时必填）；责任工程师、分析师（下拉选系统用户）；维修站号、投诉地
- **信息卡（客诉信息）**：0km 类型隐藏；支持 OCR 拍照识别自动填入各字段
- **照片上传**：最多 20 张，上传至 SMB NAS
- **售后件编号规则**：提交时生成 `{BU代码}-{产品平台代码}-{4位序号}`，如 `RBCABS-WSA-0001`
...
- 输出：PartDTO（id、partCode、productPlatform（由后端根据 partCode 自动查 APMS_PART_CODE 填充）、status=`初分析/进行中`，无 PART_NUMBER）
