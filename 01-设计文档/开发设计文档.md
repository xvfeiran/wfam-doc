...
- **基础信息录入**：零件号为文本输入，输入后自动匹配数据字典（APMS_PART_CODE）；匹配成功时产品平台自动填充且不可编辑，匹配失败时产品平台为空（两字段均为必填，未匹配则无法保存）；退货类型（BA 代码，即 COMPLAINT_TYPE）；失效类别（有失效描述时必填）；责任工程师、分析师（下拉选系统用户）；维修站号、投诉地；**关联退货单**：新建模式下仅显示状态为 `draft` 或 `submitted` 的退货单；编辑模式下，若当前关联的退货单状态非 `draft`/`submitted`，则该字段禁用并提示用户
