...
└─────────────────────────┬───────────────────────────────────┘
                          │ props: { userProfile, locale }
                          │ eventBus: locale-change
                          ▼
┌─────────────────────────────────────────────────────────────┐
│                  前端（Vue 3 子应用）                         │
...
└─────────────────────────┬───────────────────────────────────┘
                          ...
                          │ /aftermarket-parts-management-system/api/v1
                          ▼
┌─────────────────────────────────────────────────────────────┐
│                  后端（Spring Boot）                          │
...
└──────────┬──────────────────────────┬───────────────────────┘
           │ JDBC (Oracle)            │ SMB 协议 (smbj)
           ▼                          ▼
┌──────────────────┐      ┌──────────────────────────────────┐
│  Oracle Database │      │  私有 NAS（文件存储）             │
│  SUPER_LINE_LEADER│      │  图片、附件、报告模板、导出文件  │
└──────────────────┘      └──────────────────────────────────┘
                          │ HTTP（后端中转）
             ┌────────────┴─────────────┐
             ▼                          ▼
┌────────────────────┐      ┌──────────────────┐
│  自建 Dify（OCR）  │      │  SAP（零件主数据）│
│  接口待确认         │      │  接口待确认       │
└────────────────────┘      └──────────────────┘
```
...
```
用户登录 → 进入工作台
  ├── 查看数据卡片 → 点击 → 跳转对应列表（带筛选）
  ├── 查看趋势图 → 切换时间粒度 → 图表更新
  ├── 查看任务中心 → 点击任务行 → 跳转目标页面+自动筛选
  └── 点击快捷入口 → 内部跳转 / 新开标签页（外部链接）
```
...
pending_detailed_analysis（待精分析）
  ↓ [开始精分析（暂存）]
in_detailed_analysis（精分析中）◄──[审批驳回]
  ↓ [提交审批]
...
[抽样管理（弹窗）]
  选择抽样方式 → 勾选样本 → 暂存 → 确认
  ※ 确认后样本 Part 状态锁定，不可修改
      ↓
[精分析阶段] → 见 M03/M04 流程
      ↓ 审批通过后
[报废申请（弹窗）]
  填写报废信息 → 提交 → 跳转 WorkON 链接
      ↓ 用户在 WorkON 完成操作后
[确认 WorkON 已完成] → 状态变为 scrapped → completed
```
...
```
[新建售后件]
  输入零件号 → SAP 接口自动填充 BU 和产品平台
  选择投诉类型 → 若非 0km 则显示信息卡区
    ├── 手动填写车辆信息，或
    └── 点击"拍照识别" → 上传图片 → OCR 返回结果 → 逐字段"应用"或"全部应用"
  上传照片（最多20张）
  → 暂存 / 提交（生成编号，状态变 pending_initial_analysis）

[初分析]
  填写初分析内容 → 提交（状态变 initial_analysis_completed）

[精分析阶段]
  等待抽样选中（状态变 pending_detailed_analysis）
  → 见 M04 精分析报告流程
```
...
```
[录入精分析报告]
  下拉选择模板 → 请求模板 FIELDS_JSON
  → 前端动态渲染表单
  → 填写各字段（文本/下拉/日期/图片/列表）
  → 暂存草稿 / 提交审批

[审批流程]
  系统自动路由给 QMC_Leader
  → QMC_Leader 在审批管理页查看 → 通过 / 驳回（填写驳回原因）
  → 驳回后分析员收到通知，修改后重新提交

[导出报告]
  售后件详情页 → 点击"导出 Excel"
  → 后端从 NAS 读取模板 → 渲染占位符 → 返回 .xlsx 文件
```
...
[上传模板]
  点击"上传" → 选择 .xlsx 文件
  → 后端接收 → 写入 NAS（路径：templates/{名称}_{时间戳}.xlsx）
  → Apache POI 解析占位符 → 生成 FIELDS_JSON → 存入 DB
  → 前端刷新模板列表

[下载模板]
  点击"下载" → 后端从 NAS 读取文件 → 返回文件流

[删除模板]
  点击"删除" → 确认弹窗 → 后端软删除（ENABLED=0）
```
...
APMS_CUSTOMER ──────────────────────────────────────────┐
                                                         │ FK: CUSTOMER_ID
APMS_BUSINESS_UNIT ──────────────────┐                  │
  └── APMS_PRODUCT_PLATFORM          │ FK: BU/PLATFORM   │
           │                         └────────────────────┼──► APMS_RETURN_ORDER
           │                                              │         │ FK: ORDER_ID
           └────────────────────────────────────────────► APMS_PART
                                                              │
                    APMS_FAILURE_CATEGORY ──────────────┘ FK: FAILURE_CATEGORY_ID
                                                              │
                    ┌─────────────────────────────────────────┤
                    │                                         │
              APMS_PART_IMAGE                     APMS_ANALYSIS_REPORT
              (FK: PART_ID)                       (FK: PART_ID, TEMPLATE_ID)
                                                              │
...
APMS_RETURN_ORDER ──────────────────────────────────────────┐
      │                                                      │
APMS_SAMPLING ──► APMS_SAMPLING_DETAIL                APMS_SCRAP_APPLICATION
(FK: ORDER_ID)    (FK: SAMPLING_ID, PART_ID)          (FK: ORDER_ID)

APMS_STATUS_LOG  (FK: ENTITY_ID → RETURN_ORDER or PART, 逻辑外键)

APMS_REPORT_TEMPLATE (独立，被 ANALYSIS_REPORT 引用)
```
