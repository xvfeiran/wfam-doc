...
- 输出：SamplingDTO（含样本编号列表），ReturnOrder 状态变为 `初分析/完成`，相关 Part 状态变为 `精分析/进行中`
...
flowchart TD
    ...
    SubmitInitial --> Completed[状态: 初分析/完成]
...
| `初分析/完成` | 确定并确认抽样（含不抽样） | 抽样决策已完成 |
| `精分析/进行中` | 进入精分析 / 审批驳回 | 精分析报告撰写中 |
...
| 状态值 | 触发动作 | 说明 |
|------|------|------|
| `初分析/进行中` | 新建（暂存或提交） | 草稿与已提交共用此状态，通过 `PART_NUMBER IS NULL` 区分 |
| `初分析/完成` | 初分析表单提交 | 初分析完成，等待抽样 |
| `精分析/进行中` | 被抽样确认选中 / 审批驳回 | 精分析报告撰写中 |
| `精分析审批/进行中` | 提交精分析报告审批 | 等待 QMC_Leader 审批 |
| `精分析审批/完成` | QMC_Leader 审批通过 | 精分析结果已确认 |
