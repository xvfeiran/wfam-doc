...
| 范围 | 去重键 | 处理方式 |
|------|------|------|
| 文件内 | (客户编码, 收件日期, 投诉日期, 退回方式, 快递单号) | 保留首次出现行，后续相同行报错 `DUPLICATE_IN_FILE` |
| 与 DB 比对（快递类型） | 快递单号 | 快递单号已存在于 DB → 报错 `DUPLICATE_IN_DB` |
| 与 DB 比对（非快递类型） | 不做强制拦截 | 同一客户同天可能合法地存在多个退货批次 |

**验证顺序**：文件级校验（表头匹配、行数上限）→ 文件内重复检测 → 逐行格式校验 → 逐行枚举校验 → 逐行条件校验 → 批量外键校验（一次查询所有客户编码）→ DB 快递单号去重查询 → 事务批量插入有效行。
...
| v1.9 | 4.3 | 导入重复检测 | 新增 DUPLICATE_IN_FILE / DUPLICATE_IN_DB 错误类型；退货单：文件内去重键(客户+日期+方式+快递单号)，快递类型做 DB 硬拦截，非快递不拦截；售后件：文件内去重后 DB 比对采用软警告策略（允许同型号多件入档）；ImportResultDTO 新增 warningCount / warnings 字段 | 2026-03-03 |
