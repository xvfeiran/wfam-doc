...
| M04 | 精分析报告与模板系统 | /parts/:id/analysis | 分析员、分析主管、系统管理员 |
...
| W_RBCC_AEP_WFAM_QMC_Manager | QMC 经理 | M01 M02 M03 M06 |
...
- 输出：ReturnOrderDTO（id、status=`pending_registration`，无 ORDER_NUMBER）

**提交退货单：**
- 输入：id
- 输出：ReturnOrderDTO（ORDER_NUMBER=`2026EM0001`，status=`pending_initial_analysis`）

**抽样提交：**
- 输入：orderId、samplingMethod、selectedPartIds[]
- 输出：SamplingDTO（含样本编号列表），相关 Part 状态变为 `pending_detailed_analysis`

**报废申请：**
- 输入：orderId、quantity、reason、attachmentPaths[]
- 输出：ScrapApplicationDTO，状态变为 `pending_scrap`，返回 WorkON 跳转链接

##### 2.2.2.3 状态转换图（必填）

```mermaid
stateDiagram-v2
    state "草稿（无退货单号）" as pending_registration
    state "待初分析" as pending_initial_analysis
    state "待抽样" as pending_sampling
    state "抽样完成" as sampling_completed
    state "待精分析" as pending_detailed_analysis
    state "精分析中" as in_detailed_analysis
    state "待审批" as pending_approval
    state "已审批" as approved
    state "待报废" as pending_scrap
    state "已报废" as scrapped
    state "已完成" as completed

    [*] --> pending_registration: 新建暂存
    pending_registration --> pending_initial_analysis: 提交（生成退货单号）
    pending_initial_analysis --> pending_sampling: 确定抽样
    pending_sampling --> sampling_completed: 确认抽样
    sampling_completed --> pending_detailed_analysis
    pending_initial_analysis --> pending_detailed_analysis: 不抽样（跳过）
    pending_detailed_analysis --> in_detailed_analysis: 开始精分析
    in_detailed_analysis --> pending_approval: 提交审批
    pending_approval --> in_detailed_analysis: 审批驳回
    pending_approval --> approved: QMC_Leader 审批通过
    approved --> pending_scrap: 跳转 WorkON 报废
    pending_scrap --> scrapped: 确认已在 WorkON 完成
    scrapped --> completed: 自动
    completed --> [*]
```
...
flowchart TD
    ...
    ConfirmWorkON --> Done[状态: scrapped → completed]
```
...
| APMS_STATUS_LOG | 状态变更日志（entity_type=`return_order`） |
...
| 内部 | GET `/api/v1/return-orders/{id}/status-logs` | 操作日志 |
...
- 输出：PartDTO（id、status=`registered`，无 PART_NUMBER）

**提交售后件：**
- 输入：id
- 输出：PartDTO（PART_NUMBER=`BU1-PLT1-0001`，status=`pending_initial_analysis`）
...
```mermaid
stateDiagram-v2
    state "草稿（无售后件编号）" as registered
    state "待初分析" as pending_initial_analysis
    state "初分析完成" as initial_analysis_completed
    state "待精分析" as pending_detailed_analysis
    state "精分析中" as in_detailed_analysis
    state "分析完成" as analysis_completed
    state "待报废" as pending_scrap
    state "已报废" as scrapped

    [*] --> registered: 新建暂存
    registered --> pending_initial_analysis: 提交（生成售后件编号）
    pending_initial_analysis --> initial_analysis_completed: 初分析表单提交
    initial_analysis_completed --> pending_detailed_analysis: 被抽样确认选中
    pending_detailed_analysis --> in_detailed_analysis: 开始精分析
    in_detailed_analysis --> pending_detailed_analysis: 审批驳回
    in_detailed_analysis --> analysis_completed: QMC_Leader 审批通过
    analysis_completed --> pending_scrap: 报废申请提交
    pending_scrap --> scrapped: 确认 WorkON 已完成
    scrapped --> [*]
```
...
flowchart TD
    ...
    Submit -->|提交生成编号| Pending[状态: pending_initial_analysis]
    Pending --> InitialAnalysis[填写初分析内容]
    InitialAnalysis --> SubmitInitial[提交]
    SubmitInitial --> Completed[状态: initial_analysis_completed]
    Completed --> WaitSampling[等待抽样选中]
    WaitSampling --> Detailed[状态: pending_detailed_analysis\n见 M04 精分析报告流程]
```
...
| APMS_STATUS_LOG | 状态变更日志（entity_type=`part`） |
...
| 精分析报告（M04） | 售后件进入 `pending_detailed_analysis` 后触发精分析报告录入 |
...
| 内部 | GET `/api/v1/parts/{id}/status-logs` | 操作日志 |
...
**审批通过：**
- 输入：reportId
- 输出：AnalysisReportDTO（status=`approved`）；关联 Part 状态变为 `analysis_completed`

**审批驳回：**
- 输入：reportId、rejectReason
- 输出：AnalysisReportDTO（status=`rejected`）；关联 Part 状态回退到 `in_detailed_analysis`

**确认报废：**
- 输入：scrapApplicationId
- 输出：ScrapApplicationDTO（workonConfirmed=true）；关联 ReturnOrder 状态变为 `scrapped`
...
flowchart TD
    ...
    TabReport --> ListReports[列表展示 status=submitted 的报告]
...
| STATUS | VARCHAR2(50) | NOT NULL | 状态枚举（见 3.4） |
...
#### APMS_STATUS_LOG（状态变更日志）

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| ID | VARCHAR2(36) | PK | UUID |
| ENTITY_TYPE | VARCHAR2(20) | NOT NULL | return_order / part |
| ENTITY_ID | VARCHAR2(36) | NOT NULL | 关联实体 ID（逻辑外键） |
| PREVIOUS_STATUS | VARCHAR2(50) | — | 变更前状态 |
| NEW_STATUS | VARCHAR2(50) | NOT NULL | 变更后状态 |
| ACTION | VARCHAR2(50) | NOT NULL | 动作（submit/approve/reject/...） |
| OPERATOR | VARCHAR2(100) | NOT NULL | 操作人 |
...
| 枚举值 | 中文 | 说明 |
|------|------|------|
| `pending_registration` | 草稿 | 暂存，无退货单号 |
| `pending_initial_analysis` | 待初分析 | 提交后自动进入 |
...
| 枚举值 | 中文 | 说明 |
|------|------|------|
| `registered` | 草稿 | 暂存，无售后件编号 |
| `pending_initial_analysis` | 待初分析 | 提交后自动进入 |
| `initial_analysis_completed` | 初分析完成 | — |
| `pending_detailed_analysis` | 待精分析 | 被抽样选中 |
| `in_detailed_analysis` | 精分析中 | — |
...
所有状态变更（状态字段更新 + STATUS_LOG 插入）在同一事务中执行，由 Spring `@Transactional` 保证原子性。
...
| APMS_STATUS_LOG | ENTITY_ID, ENTITY_TYPE | 复合 | 操作日志查询 |
...
| 场景 | 校验逻辑 |
|------|------|
| 编辑已提交表单 | 必须是 `SystemAdmin`，否则返回 403 |
...
custom:
  ...
  analysis:
    warning-days: 14             # 精分析预警天数（进入 in_detailed_analysis 后）
